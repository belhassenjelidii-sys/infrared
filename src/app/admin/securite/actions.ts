...
import { getSession, setSessionCookie, createSessionToken } from "@/lib/auth";
...
async function limitTwoFactorVerification(action: string, userId: string) { const requestHeaders = await headers(); const limit = await consumeRateLimit(`two-factor-${action}:${clientIp(requestHeaders)}:${userId}`, 5, 5 * 60 * 1000); if (!limit.ok) throw new Error("Trop de tentatives. Réessayez dans quelques minutes."); }
export async function beginTwoFactorAction(_: TwoFactorState): Promise<TwoFactorState> { void _; try { const user = await currentUser(); const setup = await beginTwoFactorSetup(user); return setup; } catch (error) { return { error: error instanceof Error ? error.message : "Impossible de démarrer le 2FA." }; } }
