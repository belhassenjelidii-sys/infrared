...
export async function saveStaffAction(id:string|null,_state:ActionResult,f:FormData):Promise<ActionResult>{
 ...
 try{
  ...
  if((isSystemAdmin({role})||(target&&isSystemAdmin(target)))&&!isSystemAdmin(actor))throw new Error("Seul un Super Admin peut gérer ce compte.");
  ...
  await prisma.$transaction(async(tx)=>{
   if(target&&isSystemAdmin(target)&&(!active||!isSystemAdmin({role}))){const roots=await tx.user.count({where:{active:true,role:{in:["SUPER_ADMIN","DEVELOPER"]}}});if(roots<=1)throw new Error("Conservez au moins un Super Admin actif.");}
...
export async function deleteStaffAction(id:string,_state:ActionResult):Promise<ActionResult>{
 ...
 try{
  ...
  if(isSystemAdmin(target)&&!isSystemAdmin(actor))throw new Error("Seul un Super Admin peut supprimer ce compte.");
  await prisma.$transaction(async(tx)=>{if(isSystemAdmin(target)){const roots=await tx.user.count({where:{active:true,role:{in:["SUPER_ADMIN","DEVELOPER"]}}});if(roots<=1)throw new Error("Conservez au moins un Super Admin actif.");}await tx.auditLog.create({data:{actorId:actor.userId,action:"user.delete",entityType:"User",entityId:id,before:{name:target.name,email:target.email,role:target.role}}});await tx.user.delete({where:{id}});});
