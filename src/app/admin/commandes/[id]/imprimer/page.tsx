...
export default async function PrintOrderPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{type?:string}>}) {
  ...
  return <main className="print-document mx-auto min-h-screen max-w-5xl bg-white p-5 text-[#172033] sm:p-10">
    ...
    <article className="print-sheet overflow-hidden rounded-2xl border border-[#d8dee8] bg-white shadow-sm print:rounded-none print:shadow-none">
      ...
      <section className="grid gap-px bg-[#d8dee8] sm:grid-cols-3"><div className="bg-[#f7f9fc] p-5"><p className="document-label">Client</p><p className="mt-2 font-bold">{String(customer.name??"—")}</p><p className="text-sm">{String(customer.phone??"—")}</p></div><div className="bg-[#f7f9fc] p-5"><p className="document-label">Livraison</p><p className="mt-2 font-bold">{fulfillment.method==="DELIVERY"?"Livraison à domicile":"Retrait en boutique"}</p><p className="text-sm">{String(fulfillment.address??"")}</p><p className="text-sm">{String(fulfillment.city??"")}</p></div><div className="bg-[#f7f9fc] p-5"><p className="document-label">{delivery?"Transporteur":"Traitement"}</p>{delivery?<><p className="mt-2 font-bold">{courierName}</p><p className="text-sm">{courierPhone}</p><p className="text-sm">{order.trackingNumber?`Suivi : ${order.trackingNumber}`:""}</p></>:<><p className="mt-2 font-bold">{paymentLabel(order.paymentMethod)}</p><p className="text-sm">{statusLabel[order.status]??order.status}</p></>}</div></section>
