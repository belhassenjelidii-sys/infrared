...
export default async function PrintOrderPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{type?:string}>}) {
  ...
  const [order,settings]=await Promise.all([
    prisma.order.findUnique({where:{id},include:{preparationStore:true,deliveryCompany:true,items:{include:{variant:{include:{brand:true,images:{take:1,orderBy:{sortOrder:"asc"}}}}}}}}),
  ...
  const pickupStore=object(fulfillment.store);
  const fulfillmentTitle=fulfillment.method==="DELIVERY"?"Livraison à domicile":"Retrait en boutique";
  const fulfillmentPrimary=fulfillment.method==="DELIVERY"?String(fulfillment.address??""):String(pickupStore.name??"Boutique non renseignée");
  const fulfillmentSecondary=fulfillment.method==="DELIVERY"?String(fulfillment.city??""):String(pickupStore.address??"");
  const itemCount=order.items.reduce((sum,item)=>sum+item.quantity,0);
  const courierName=order.deliveryCompany?.name??([order.manualDeliveryFirstName,order.manualDeliveryLastName].filter(Boolean).join(" ")||"Non affecté");
  const courierPhone=order.deliveryCompany?.phone??order.manualDeliveryPhone??"";
  ...
  return <main className="print-document mx-auto min-h-screen max-w-5xl bg-white p-5 text-[#172033] sm:p-10">
    ...
    <article className="print-sheet overflow-hidden rounded-2xl border border-[#d8dee8] bg-white shadow-sm print:rounded-none print:shadow-none">
      <header className="flex flex-wrap items-start justify-between gap-6 bg-[#0a1421] px-7 py-6 text-white">
        <div className="flex items-center gap-4"><img src={logoUrl} alt="InfraRed Optic Store" className="h-16 w-auto max-w-48 object-contain"/><div><h1 className="text-xl font-bold">InfraRed Optic Store</h1><p className="mt-1 max-w-xs text-xs text-slate-300">{settings?.address??"Tunis, Tunisie"}</p><p className="text-xs text-slate-300">{settings?.phone??settings?.whatsapp??""}</p></div></div>
        <div className="text-right"><p className="text-[11px] font-bold uppercase tracking-[.22em] text-[#ff5264]">{delivery?"Bordereau de livraison":"Bon de commande"}</p><h2 className="mt-2 text-2xl font-black">{order.number}</h2><p className="mt-1 text-xs text-slate-300">Date : {order.createdAt.toLocaleDateString("fr-TN")}</p></div>
      </header>

      <section className="grid gap-px bg-[#d8dee8] sm:grid-cols-4"><div className="bg-[#f7f9fc] p-5"><p className="document-label">Client</p><p className="mt-2 font-bold">{String(customer.name??"—")}</p><p className="text-sm">{String(customer.phone??"—")}</p></div><div className="bg-[#f7f9fc] p-5"><p className="document-label">Livraison</p><p className="mt-2 font-bold">{fulfillmentTitle}</p><p className="text-sm">{fulfillmentPrimary}</p><p className="text-sm">{fulfillmentSecondary}</p></div><div className="bg-[#f7f9fc] p-5"><p className="document-label">Boutique de préparation</p><p className="mt-2 font-bold">{order.preparationStore?.name??"Non affectée"}</p><p className="text-sm">{order.preparationStore?.address??""}</p></div><div className="bg-[#f7f9fc] p-5"><p className="document-label">{delivery?"Transporteur":"Traitement"}</p>{delivery?<><p className="mt-2 font-bold">{courierName}</p><p className="text-sm">{courierPhone}</p><p className="text-sm">{order.trackingNumber?`Suivi : ${order.trackingNumber}`:""}</p></>:<><p className="mt-2 font-bold">{paymentLabel(order.paymentMethod)}</p><p className="text-sm">{statusLabel[order.status]??order.status}</p></>}</div></section>
      ...
      <div className="p-7"><div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold">Détail des articles</h3><span className="rounded-full bg-[#eef1f6] px-3 py-1 text-xs font-semibold">{itemCount} article{itemCount>1?"s":""}</span></div>
        ...
        <section className="signature-client mt-8"><p className="document-label">Signature du client</p><div className="mt-3 h-28 rounded-lg border border-dashed border-[#9ca7b6] bg-[#fbfcfe]"/></section>
      </div>
    </article>
  </main>;
}
