...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (paymentId) {
      const existing = await prisma.order.findFirst({ where: { externalPaymentId: paymentId }, select: { number: true } });
      if (existing) return NextResponse.redirect(new URL(`/commande/${existing.number}`, url.origin));
    }
    const orderNumber = await finalizeTndPaymentSession(sessionId, paymentId);
    (await cookies()).delete(CART_COOKIE);
    return NextResponse.redirect(new URL(`/commande/${orderNumber}`, url.origin));
  } catch (error) {
