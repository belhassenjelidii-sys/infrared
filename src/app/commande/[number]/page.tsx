...
export default async function OrderConfirmation({params,searchParams}:{params:Promise<{number:string}>;searchParams:Promise<{token?:string}>}) {
  ...
  const order=await prisma.order.findFirst({where:{number,publicToken:token},select:{number:true,total:true,items:{select:{id:true,quantity:true,brandName:true,productName:true,reference:true,frameColor:true,size:true,unitPrice:true}}}});
  if(!order) notFound();
